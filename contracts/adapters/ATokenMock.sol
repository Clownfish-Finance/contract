...
contract ATokenMock is IAToken, ERC20 {
    ...
    constructor(
        ...
        ERC20(
            string(
                abi.encodePacked("Aave interest bearing ", ERC20(_asset).name())
            ),
    ...
    // function _transfer(address from, address to, uint256 amount) public override {
    //     super._transfer(from, to, _divByIndex(amount, _index()));
    // }

    function transferCustom(address from, address to, uint256 amount) public {
        // 自定义操作逻辑
        super._transfer(from, to, _divByIndex(amount, _index()));
    }
